...
<%-- 						   <img src="/fileupload/${item.atchmnfl_mask_nm}" class="card-img-top" alt="..."> --%>
				   ...
				   <div class="container" style="margin-left: auto; margin-right: none;">
					    ...
					    <a class="btn col-xl-2 col-md-2" href="/board/regist?seq=${sangaView.seq}">수정</a>
			        	    <c:choose>
					   	    	<c:when test="${sangaView.file_id eq 0 }">
					   	    		<a class="btn col-xl-2 col-md-2" href="/board/deleteproc?seq=${sangaView.seq}">삭제</a>
					   	    	</c:when>
					   	    	<c:when test="${sangaView.file_id ne 0 }">
					   	    		<a class="btn col-xl-2 col-md-2" href="/board/deleteproc?seq=${sangaView.seq}&file=${sangaImageView.get(0).file_id}">삭제</a>
					   	    	</c:when>
					   	    </c:choose>       	   
					    </c:when>
					 </c:choose>
				    ...
				    <c:forEach var="comment" items="${tourCommentRank}">
				    	<p>${comment.reg_user_id}</p>
				    	<p>${comment.rank_comment}</p>
				    	<p>${comment.rank}</p>
				    	<p>${comment.reg_dt}</p>
				    </c:forEach>
